...
The configuration values can be composed together to create an arbitrarily complex 
custom browser.
The majority of major genomic data types displayed in genome browsers are supported.
JBrowseR also includes a HTTP server for serving local data that is configured with
...
In order to demonstrate the utility of JBrowseR, several Shiny apps were built and included along
...
file like the other JBrowse 2 products by loading the Sars-CoV-2 reference genome and NCBI annotations
...
It considerably lowers the level of web development expertise required to create a genomics 
application with a fast and flexible genome browser. 
However, applications are not the only available target point for the rendering functions.
JBrowseR can embed a genome browser into R Markdown, which is a flexible documentation format
that is widely used to write scientific articles.
...
The JBrowseR is freely available for download from CRAN, and the source code is publicly available at
